...
+ Instance Store
 + Instance Store는 EC2 생성 시 temporary block storage를 제공합니다. 이 스토리지는 host computer에 물리적으로 연결된 디스크에 위치하기 때문에 EBS에 연결된 것보다 월등히 빠른 성능을 제공합니다. 버퍼나 캐시, 스크래치 데이터 및 기타 임시 컨텐츠와 같이 자주 변경되는 정보의 임시 스토리지나 로드밸런싱된 웹서버pool과 같이 인스턴스 플릿에서 복제되는 데이터에 적합.
